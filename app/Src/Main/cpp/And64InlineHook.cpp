...
static struct {
    ...
    uint32_t code[4];
    bool used;
} g_backups[A64_MAX_BACKUPS] = {};

static int Protect(void* addr, size_t sz, int prot) {
    return mprotect((void*)PAGE_ALIGN((uintptr_t)addr), PAGE_SIZE, prot);
}
...
void A64HookFunction(void* target, void* newFunc, void** original) {
    ...
    *original = target;
    ...
    int slot = -1;
    for (int i = 0; i < A64_MAX_BACKUPS; i++) {
        if (!g_backups[i].used) { slot = i; break; }
    }
    if (slot == -1) return;
